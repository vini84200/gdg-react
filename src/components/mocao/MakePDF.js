...
const getTitle = (num, year, type) => {
  if (type === "proposta") return `PROJETO DE RESOLUÇÃO Nº ${num}, DE ${year}`;

  if (type === "resolucao") return `RESOLUÇÃO Nº ${num}, DE ${year}`;

  if (type === "proposta_emenda")
    return `PROJETO DE EMENDA EM RESOLUÇÃO Nº ${num}, DE ${year}`;

  if (type === "emenda") return `EMENDA EM RESOLUÇÃO Nº ${num}, DE ${year}`;
...
export default async resolucao => {
  const documentDefinition = {
    content: [
      ...
      {
        text:
          resolucao.chamada || "O Grêmio Estudantil Vinícius de Moraes resolve:"
      },
      {
        layout: "noBorders",
        table: {
      ...
      renderAssinaturaPDF(
        ...
        resolucao.assinante || "Proponente"
      )
    ]
  };
